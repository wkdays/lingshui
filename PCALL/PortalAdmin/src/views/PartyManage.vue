<template>
  <div class="page-card">
    <div class="page-header">
      <span class="page-title">党建工作管理</span>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑党建' : '新增党建'" width="900px" destroy-on-close>
...
import { partyApi, uploadFile } from '../api'
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await partyApi.list({ page: page.value, limit: limit.value })
    list.value = res.data
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await partyApi.update(editId.value, form)
...
const handleToggleRecommend = async (row) => {
  await partyApi.update(row.id, { recommend: row.recommend ? 0 : 1 })
  ElMessage.success('操作成功')
  loadList()
}

const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除该党建内容吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    await partyApi.delete(row.id)
