<template>
  <div class="page-card">
    <div class="page-header">
      <span class="page-title">招标公告管理</span>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑公告' : '新增公告'" width="900px" destroy-on-close>
...
import { biddingApi, uploadFile } from '../api'
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await biddingApi.list({ page: page.value, limit: limit.value })
    list.value = res.data
...
const handleAdd = () => {
  isEdit.value = false
  editId.value = null
  Object.assign(form, { title: '', cover: '', summary: '', content: '' })
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await biddingApi.update(editId.value, form)
      ElMessage.success('更新成功')
    } else {
      await biddingApi.create(form)
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除该公告吗？', '提示', {
    ...
    await biddingApi.delete(row.id)
